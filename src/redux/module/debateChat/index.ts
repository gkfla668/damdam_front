...
import { IDebateChatToJSON } from 'types/debate/info'
...
const GetRoomOne = (debateId: string) => async (dispatch: any) => {
	dispatch(actions.REQUEST())
	const res = await axios
		.get(`/@api/debate/room/${debateId}`)
		.then((res) => res)
...
const GetChatList = (roomId: string, params?: any) => async (dispatch: any) => {
	dispatch(actions.REQUEST())
	const res = await axios
		.get(`/@api/debate/message/${roomId}`, { params })
		.then((res) => res)
		.catch((err) => err.response)
...
const AddChatOne = (data: IDebateChatToJSON) => async (dispatch: any) => dispatch(actions.ADD_CHAT_ONE(data))
const UpdateRoomInfo = (data: any) => async (dispatch: any) => dispatch(actions.SET_ROOM(data))

export const DebateChatAction = {
	INIT,
	INIT_STATUS,
	//-Actions
	GetRoomOne,
